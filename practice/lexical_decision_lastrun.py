...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/yoon/psychopy/PsychoPy-101/practice/lexical_decision_lastrun.py',
...
text_show = visual.TextStim(win=win, name='text_show',
    text='default text',
    font='Arial',
    pos=[0,0], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
...
for thisTrial in trials:
    ...
    text_show.setPos((0, 0))
    text_show.setText(engText)
    # keep track of which components have finished
    TaskComponents = [task_resp, text_show]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *text_show* updates
        if t >= 0.0 and text_show.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_show.tStart = t  # not accounting for scr refresh
            text_show.frameNStart = frameN  # exact frame index
            win.timeOnFlip(text_show, 'tStartRefresh')  # time at next scr refresh
            text_show.setAutoDraw(True)
        frameRemains = 0.0 + 5- win.monitorFramePeriod * 0.75  # most of one frame period left
        if text_show.status == STARTED and t >= frameRemains:
            # keep track of stop time/frame for later
            text_show.tStop = t  # not accounting for scr refresh
            text_show.frameNStop = frameN  # exact frame index
            win.timeOnFlip(text_show, 'tStopRefresh')  # time at next scr refresh
            text_show.setAutoDraw(False)
    ...
    trials.addData('text_show.started', text_show.tStartRefresh)
    trials.addData('text_show.stopped', text_show.tStopRefresh)
    thisExp.nextEntry()
